...
> 算例描述+数据集
> 一个折叠翼实验算例被用于进一步验证本文方法，该折叠翼结构主要由前翼板、后翼板、转轴和锁紧销组成如图⭐。在建立的有限元模型中，锁紧销
> 折叠翼的动力学特性受多个设计参数影响，包括前翼板厚度$t_{1}$、后翼板厚度$t_{2}$、转轴直径$d$、前翼板长度$L_{1}$和后翼板长度$L_{2}$。通过有限元仿真生成了包含不同设计参数组合的频率响应函数（FRF）数据集，用于训练和测试代理模型。
> 该算例涉及一个折叠翼结构，其动力学特性受到多个设计参数的影响。通过有限元仿真生成了包含不同设计参数组合的FRF数据集，用于训练和测试代理模型。

The proposed AE-MLP framework is further validated on a folding fin structure 
